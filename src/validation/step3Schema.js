...
export const step3Schema = yup.object({
	additional: yup.object({
		...
		// 	.date()
		// 	.nullable()
		// 	.typeError('Дата обовʼязкова')
		// 	.required('Дата народження обовʼязкова')
		// 	.test('age', 'Вік має бути від 18 до 100 років', value => {
		...
		birthDate: yup
			...
			.test('age', 'Вік має бути від 18 до 100 років', value => {
				if (!value) return false;
				return (
					isAdult(new Date(value)) >= 18 &&
					isAdult(new Date(value)) <= 100
				);
			}),
		...
		bio: yup.string().max(500, 'Максимум 500 символів')
	})
});
